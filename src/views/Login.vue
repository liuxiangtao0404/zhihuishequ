		...
		methods:{
			denglu() {
				if (this.user != "" || this.pwd != "") {
					this.axios.post("http://139.9.169.87:8081/examsystem/login?username="+this.user+"&password="+this.pwd)
					.then(res => {
						this.$message({ message: '登陆成功',type: 'success'});
						localStorage.setItem('zhuser', this.user)
						localStorage.removeItem('zhpwd')
